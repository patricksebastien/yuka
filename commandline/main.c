 ...
 * Modify by: Patrick Sébastien for workinprogress project:
 * http://www.workinprogress.ca/wip/
 */
...
//Default
static int IDENT_VENDOR_NUM = 0x16c0;
static char IDENT_VENDOR_STRING[255] = "obdev.at";
static int IDENT_PRODUCT_NUM = 0x05df;
static char IDENT_PRODUCT_STRING[255] = "HIDBoot";
...
char    *file = NULL;
    ...
    if(strcmp(argv[1], "-r") == 0){
		...
		long val_vn = strtol(argv[3], &end_vn, 16);
		IDENT_VENDOR_NUM = val_vn;
        strcpy(IDENT_VENDOR_STRING, argv[4]);
		
		long val_pn = strtol(argv[5], &end_pn, 16);
		IDENT_PRODUCT_NUM = val_pn;
		strcpy(IDENT_PRODUCT_STRING, argv[6]);
		
    }else{
        file = argv[1];
		long val_vn = strtol(argv[2], &end_vn, 16);
		IDENT_VENDOR_NUM = val_vn;
		strcpy(IDENT_VENDOR_STRING, argv[3]);
		long val_pn = strtol(argv[4], &end_pn, 16);
		IDENT_PRODUCT_NUM = val_pn;
		strcpy(IDENT_PRODUCT_STRING, argv[5]);
    }
